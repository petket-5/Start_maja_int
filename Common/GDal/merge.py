#!/home/akynos/anaconda2/envs/tf_gpu_19/bin/python
###############################################################################
...
class file_info:
    ...
    def init_from_name(self, dataset):
        """
        Initialize file_info from filename

        dataset -- Dataset of file to read.

        Returns 1 on success or 0 if the file can't be opened.
        """
        if type(dataset) == gdal.Dataset:
            fh = dataset
        elif type(dataset) == str:
            fh = gdal.Open(dataset)
        else:
            raise RuntimeError("Unknown type encountered %s" % type(dataset))
        if fh is None:
            return 0
...
def run_merge(*src, argv=None):
    ...
    if out_file is None:
        format = "MEM"
        out_file = "/vsimem/" + uuid.uuid4().hex
    if format is None:
